...
The preprocessed time-dependent dataset used in this report is stored as  
`data/LUMIERE_TD_FINAL.csv`. A companion script (`scripts/01_preprocess_to_TD_FINAL.R`)  
demonstrates how this file can be derived from the original LUMIERE CSV files.
...
```{r load_data}
# Load the final time-dependent dataset for analysis
library(readr)
library(here)
...
- The main analysis scripts are provided under `scripts/`:
  - `01_preprocess_to_TD_FINAL.R`: preprocessing pipeline to derive `LUMIERE_TD_FINAL.csv`
  - `02_run_survival_models.R`: fits the final Cox model
  - `03_plot_results.R`: creates time-dependent performance and risk-group KM plots
  - `04_export_results.R`: exports summary tables (C-index, IBS, etc.)
  - `05_render_report.R`: renders this R Markdown file to HTML
