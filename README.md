...
assess time-dependent discrimination and calibration (C-index, Brier score, IBS).This repository demonstrates an independent outcome-driven survival modeling framework for longitudinal imaging data.
...
.
├── README.md
├── LUMIERE-survival-report.Rmd     # Main R Markdown report
├── scripts/
│   ├── 01_preprocess_to_TD_FINAL.R # From raw LUMIERE CSVs to survival-ready TD dataset
│   ├── 02_run_survival_models.R    # Fits Cox models and saves intermediate objects
│   ├── 03_plot_results.R           # Generates figures (C-index, Brier, KM, heatmaps)
│   ├── 04_export_results.R         # (optional) Export summary tables, metrics
│   └── 05_render_report.R          # (optional) Renders the Rmd report programmatically
├── data/                           # Raw and derived data (NOT tracked in Git)
│   └── README_data.md              # Instructions for obtaining the LUMIERE CSV files
├── results/                        # Auto-generated results (figures, RDS, CSV; Git-ignored)
└── .gitignore
...
1️⃣ Preprocessing: from raw CSVs to survival-ready dataset
...
2️⃣ Fit Cox models and save intermediate objects
...
4️⃣ Render the R Markdown report (optional)
